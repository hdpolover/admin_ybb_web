...
<head>
    <?php echo view('partials/title-meta', array('title' => 'Gender Analytics')); ?>
...
<body>
        ...
        <div class="main-content">
            <div class="page-content">
                <div class="container-fluid">
                    <?php echo view('partials/page-title', array('pagetitle' => 'Analytics', 'title' => 'Gender Analytics')); ?>
                            ...
                            <div class="card card-animate card-height-100">
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 overflow-hidden ms-3">
                                            <p class="text-uppercase fw-medium text-muted text-truncate mb-1">Male</p>
                                            <h4 class="fs-4 flex-grow-1 mb-1"><?= $malePct ?>%</h4>
                            ...
                            <div class="card card-animate card-height-100">
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 overflow-hidden ms-3">
                                            <p class="text-uppercase fw-medium text-muted text-truncate mb-1">Female</p>
                                            <h4 class="fs-4 flex-grow-1 mb-1"><?= $femalePct ?>%</h4>
                                            <p class="text-muted mb-0">Of all participants</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body text-center py-5">
                                        ...
                                        <h5 class="mt-3 text-muted">No Gender Data Available</h5>
                                        <p class="text-muted">There is no gender data for the selected program yet.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-xl-5">
                            <div class="card card-height-100">
                                ...
                                <div class="card-body">
                                    <div id="gender-donut-chart" class="apex-charts" style="height: 380px;"></div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-7">
                            <div class="card card-height-100">
                                <div class="card-header align-items-center d-flex">
                                    <h4 class="card-title mb-0 flex-grow-1">Gender Breakdown (Bar)</h4>
                                </div>
                                <div class="card-body">
                                    <div id="gender-bar-chart" class="apex-charts" style="height: 380px;"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header align-items-center d-flex">
                                    <h4 class="card-title mb-0 flex-grow-1">Gender × Nationality</h4>
                                    <div class="flex-shrink-0 d-flex align-items-center gap-2">
                                        <label class="mb-0 me-1 text-muted" for="gn-filter">Filter by Gender:</label>
                                        <select id="gn-filter" class="form-select form-select-sm" style="min-width: 160px;">
                                            <option value="">All Genders</option>
                                            <?php foreach (array_column($nationalityMatrixRows, 'label') as $g): ?>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        <table id="gn-datatable" class="table table-bordered table-striped matrix-table dt-responsive w-100">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header align-items-center d-flex">
                                    <h4 class="card-title mb-0 flex-grow-1">Gender × Age Band</h4>
                                    <div class="flex-shrink-0 d-flex align-items-center gap-2">
                                        <label class="mb-0 me-1 text-muted" for="ga-filter">Filter by Gender:</label>
                                        <select id="ga-filter" class="form-select form-select-sm" style="min-width: 160px;">
                                            <option value="">All Genders</option>
                                            <?php foreach (array_column($ageMatrixRows, 'label') as $g): ?>
                                                <option value="<?= esc($g) ?>"><?= esc($g) ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                ...
                                <div class="card-body">
                                    <?php if (empty($ageMatrixRows)): ?>
                                        <p class="text-center text-muted py-4">No data available.</p>
                                    <?php else: ?>
                                    <div class="table-responsive">
                                        <table id="ga-datatable" class="table table-bordered table-striped matrix-table dt-responsive w-100">
                                            <thead class="table-light">
                                                <tr>
                                                    <th>Gender</th>
                                                    <?php foreach ($ageCols as $col): ?>
                                                        <th><?= esc($col) ?></th>
                                                    <?php endforeach; ?>
                                                    <th class="row-total">Total</th>
                                                </tr>
                                            ...
                                            <tbody>
                                                <?php foreach ($ageMatrixRows as $row): ?>
                                                    <tr>
                                                        <td><?= esc($row['label']) ?></td>
                                                        <?php foreach ($ageCols as $col): ?>
                                                            <td class="text-center"><?= number_format($row['counts'][$col] ?? 0) ?></td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var gData = <?= $genderChartData ?>;

            var chartColors = ['#038edc', '#f7b84b', '#51d28c', '#f34e4e', '#564ab1'];

            // Pie chart
            if (document.getElementById('gender-donut-chart') && gData.labels[0] !== 'No Data') {
                new ApexCharts(document.querySelector('#gender-donut-chart'), {
            ...
            if (document.getElementById('gender-bar-chart') && gData.labels[0] !== 'No Data') {
                new ApexCharts(document.querySelector('#gender-bar-chart'), {
                    series: [{ name: 'Participants', data: gData.values }],
                    chart: { type: 'bar', height: 380, toolbar: { show: false } },
                    plotOptions: { bar: { borderRadius: 4, horizontal: true, distributed: true } },
                    dataLabels: { enabled: false },
                    colors: chartColors,
                    xaxis: { categories: gData.labels, title: { text: 'Number of Participants' } },
                    noData: { text: 'No data available', align: 'center', verticalAlign: 'middle', style: { color: '#6c757d', fontSize: '16px' } }
                }).render();
            }
            ...
            initMatrix('gn-datatable', 'gn-filter');
            initMatrix('ga-datatable', 'ga-filter');
        });
    </script>
